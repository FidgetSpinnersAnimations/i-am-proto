...
class PlayState extends MusicBeatState
{
	...
	private var spacePresses:Int = 0;
	
	private var shakeScreen:Bool = false;
	private var blackAlpha:FlxSprite;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'bg':
				{
						defaultCamZoom = 0.9;
						curStage = 'bg';
						background = new FlxSprite(-700, -400).loadGraphic(Paths.image('i-am-proto/bg'));
						background.setGraphicSize(Std.int(background.width * 0.98));
			...
			case 'bg-glitch': 
			{
				...
				var texr = Paths.getSparrowAtlas('i-am-proto/bg-glitch');

				background = new FlxSprite(-700, -700);
				background.frames = texr;
				background.animation.addByPrefix('idle', 'glitch');
		...
		if (SONG.song.toLowerCase() == 'copier')
		{
			boyfriend.y -= 170;
		}
		...
		if (SONG.song.toLowerCase() == 'copier')
		{
			healthBar.createFilledBar(0xFF00CE22, 0xFFFF6600);
		}
		else if (SONG.song.toLowerCase() == 'roses-and-viruses')
		{
			healthBar.createFilledBar(0xFFF6AF72, 0xFFFF6600);
		}
		else
		{
	    	healthBar.createFilledBar(0xFFFF6600, 0xFF0099FF);
		}
		// healthBar
		add(healthBar);
		...
		spaceTxt = new FlxText(FlxG.width + 500, healthBarBG.y - 100, 0, "SPAM SPACE BEFORE THE VIRUS TAKES OVER: " + spacePresses, 40);
		
		spaceTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.RED, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);

		spaceTxt.screenCenter(X);
		...
		spaceTxt.scrollFactor.set();
		
		spaceTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);

		if (SONG.song.toLowerCase() == 'installation')
	    {
			add(spaceTxt);
		}
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
...

			{
				...
				case 3:
					...
					FlxG.sound.play(Paths.sound('error' + altSuffix), 0.6);
				case 4:
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'installation')
		{
			if (FlxG.keys.justPressed.SPACE && spacePresses < 100)
			{
				spacePresses += FlxG.random.int(1, 2);
				remove(spaceTxt);
                spaceTxt.text = ("DOWNLOADING PROTEGENT: " + spacePresses + "%");
				add(spaceTxt);
			}
			if (spacePresses > 99)
			{
				spaceTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.GREEN, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
			}
		}
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				#end
				if (SONG.player1 == 'proto')
				{
			    	camFollow.setPosition(boyfriend.getMidpoint().x - 100 + offsetX, boyfriend.getMidpoint().y - 100 + offsetY);
				}
				else
				{
			    	camFollow.setPosition(boyfriend.getMidpoint().x - 100 + offsetX, boyfriend.getMidpoint().y - 250 + offsetY);
				}
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						daNote.destroy();
						if (SONG.song.toLowerCase() == 'protegent')
						{
					    	health -= 0.010;
						}
						if (SONG.song.toLowerCase() == 'installation')
						{
					    	FlxG.camera.shake(0.020, 0.020);
						}
					}
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
			    	...
			    	{
			     		case 'protegent':
                            video.playMP4(Paths.video('virus'));
                            video.finishCallback = function()
                            {
                             	LoadingState.loadAndSwitchState(new PlayState());
                            }
			    		case 'installation':
                            video.playMP4(Paths.video('install'));
                            video.finishCallback = function()
                            {
                             	LoadingState.loadAndSwitchState(new PlayState());
                            }
	...
	*/
	...
	function error()
	{
		var error:FlxSprite = new FlxSprite(FlxG.random.int(300, 1077), FlxG.random.int(0, 622));
		if (FlxG.random.bool(20))
		{
			error.loadGraphic(Paths.image('i-am-proto/ERROR' + FlxG.random.int(8, 10), 'shared'));
			error.setGraphicSize(Std.int(error.width * 0.9));
		}
		else
		{
			error.loadGraphic(Paths.image('i-am-proto/ERROR' + FlxG.random.int(1, 7), 'shared'));
			error.setGraphicSize(Std.int(error.width * 1.5));
		}
		error.updateHitbox();
	...
	{
		var error:FlxSprite = new FlxSprite(FlxG.random.int(300, 1077), FlxG.random.int(0, 622));
		error.loadGraphic(Paths.image('i-am-proto/ERRORNOTEMESSAGE', 'shared'));
		error.setGraphicSize(Std.int(error.width * 1.2));
		error.updateHitbox();
		error.antialiasing = true;
		add(error);
		FlxG.sound.play(Paths.sound("error"));
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'antivirus')
		{
			switch(curStep)
			{
				case 340:
		...
		if (SONG.song.toLowerCase() == 'installation')
		{
			switch(curStep)
			{
			    ...
			    case 300:
					FlxTween.tween(dad, {y: -200, alpha: 0}, 15);
			}
		}
		
		if (SONG.song.toLowerCase() == 'roses-and-viruses')
		{
			switch(curStep)
			{
				case 512:
					remove(dad);
					dad = new Character(210, 370, 'senpai-angry');
					add(dad);
				case 800:
					SONG.speed = 2.5;
				case 832:
					blackAlpha = new FlxSprite(0, 0).loadGraphic(Paths.image('i-am-proto/black_alpha'));
					blackAlpha.alpha = 0;
					blackAlpha.cameras = [camHUD];
					blackAlpha.screenCenter();
					shakeScreen = true;
					add(blackAlpha);
					FlxTween.tween(blackAlpha, {alpha: 1}, 5, {ease: FlxEase.cubeIn, });
				case 960:
					FlxTween.tween(blackAlpha, {alpha: 0}, 2, {ease: FlxEase.cubeIn, });
					shakeScreen = false;
			}
		}
		...
		if (SONG.song.toLowerCase() == 'copier')
		{
			switch(curStep)
			{
				case 767:
					remove(dad);
					dad = new Character(100, 450, 'bf');
					add(dad);
					dad.alpha = 1;
					iconP2.animation.play('bf-nodeath');
					FlxG.camera.flash(FlxColor.WHITE, 0.2);
				case 895:
					dad.playAnim('idle');
				case 920:
					iconP2.animation.play('bf-dead');
					dad.playAnim('singUP');
                    new FlxTimer().start(0.1, function(tmr:FlxTimer)
                        {
                            dad.alpha -= 0.1;
				...
				case 1000:
					remove(dad);
					dad = new Character(60, 140, 'why');
					dad.alpha = 0;
					add(dad);
				case 1023:
					iconP2.animation.play('why');
		...
		if (SONG.song.toLowerCase() == 'protegent')
		{
			...
			if (curStep == 392 || curStep == 393 || curStep == 394 || curStep == 395 || curStep == 396 || curStep == 397 || curStep == 398 || curStep == 399)
			{
				boyfriend.playAnim('hey');
			}
			if (curStep == 576)
		...
		if (SONG.song.toLowerCase() == 'installation')
		{
            if (curStep % 4 == 0 && spacePresses < 100)
			{
				boyfriend.playAnim('protegent');
			}
		}
